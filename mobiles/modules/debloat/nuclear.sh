...
run_nuclear() {
  ...
  section "debloat/nuclear — uninstalling system packages"
  ...
  while IFS= read -r pkg; do
    ...
    local result
    result=$(adb shell pm uninstall --user 0 "$pkg" </dev/null 2>&1) || true
  ...
  echo ""
  echo "Uninstalled: $OK  |  Kept: $SKIP  |  Failed: ${#FAIL[@]}"
